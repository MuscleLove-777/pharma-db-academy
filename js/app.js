...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('pharma-db-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('pharma-db-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('pharma-db-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('pharma-db-progress', JSON.stringify(this.progress));
        localStorage.setItem('pharma-db-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>商用DB Academy へようこそ</h2>
                    <p>製薬業界で活用される商用データベースの活用方法を基礎から実践まで学べるプラットフォームです。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('pharma-db-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('pharma-db-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('pharma-db-progress');
            localStorage.removeItem('pharma-db-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
